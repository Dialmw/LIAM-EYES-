...
// ─────────────────────────────────────────────────────────────────────────────
{
    ...
    execute: async (sock, m, { args, reply }) => {
        ...
        try {
            ...
            reply(`🕒 *Time (${tz})*\n\n${t}\n\n${sig()}`);
        } catch(e) { reply(`❌ Invalid timezone. Example: *.time Africa/Nairobi*\n\n${sig()}`); }
    }
},
...
// ─────────────────────────────────────────────────────────────────────────────
{
    ...
    execute: async (sock, m, { pushname, reply }) => {
        const list = [
            `You light up every room you enter, ${pushname}! ✨`,
            `${pushname}, you're more fun than a bag of kittens! 🐱`,
            `If kindness was currency, ${pushname} would be a billionaire! 💰`,
            `${pushname}, you make the world a better place just by being in it! 🌍`,
            `You have a smile that could stop traffic, ${pushname}! 😊`,
            `${pushname} — your positivity is contagious and we love it! 💪`,
        ];
        await react(sock, m, '💝');
        reply(`💝 *Compliment for ${pushname}*\n\n${list[Math.floor(Math.random()*list.length)]}\n\n${sig()}`);
    }
},
...
// ─────────────────────────────────────────────────────────────────────────────
{
    ...
    execute: async (sock, m, { pushname, reply }) => {
        const list = [
            `${pushname}, you're proof that evolution can go in reverse. 😂`,
            `${pushname}, your Wi-Fi password is probably "password123" isn't it? 😅`,
            `${pushname}, I've seen better software in a broken calculator! 💻`,
            `${pushname}, your fashion sense called — it said it's lost. 👗`,
            `${pushname}, if brains were Wi-Fi, you'd still be on 2G. 📶`,
        ];
        await react(sock, m, '🔥');
        reply(`🔥 *Playful Roast*\n\n${list[Math.floor(Math.random()*list.length)]}\n\n_All in good fun! 😄_\n\n${sig()}`);
    }

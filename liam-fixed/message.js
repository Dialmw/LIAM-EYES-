...
module.exports = async (sock, m, chatUpdate, store) => {
    try {
        ...
        // reply WITHOUT thumbnail — clean text reply for all commands
        ...
        if (!m.key.fromMe && !isCmd && /\bbot\b/i.test(body)) {
            ...
            const botTxt =
                ...
                `> 𝐋𝐈𝐀𝐌 𝐄𝐘𝐄𝐒 👁️`;
            try {
                await sock.sendMessage(m.chat, { text: botTxt }, { quoted: m });
            } catch (_) {
        ...
        if (command === 'menu' || command === 'help') {
            ...
            if (style === 2) {
                const txt2 = header + PL.style2(prefix) + styleHint;
                if (image && image.length > 0) { await sock.sendMessage(m.chat, { image, caption: txt2 }, { quoted: m }); }
                else { await replyPlain(txt2); }
                return;
            }

            // ── STYLE 3 — classic box (matches sample image) ─────────────────
            if (style === 3) {
                const txt3 = classicHdr + PL.style3(prefix) + styleHint;
                if (image && image.length > 0) { await sock.sendMessage(m.chat, { image, caption: txt3 }, { quoted: m }); }
                else { await replyPlain(txt3); }
                return;
            }

            // ── STYLE 4 — cursive flower ─────────────────────────────────────
            if (style === 4) {
                const txt4 = header + PL.style4(prefix) + styleHint;
                if (image && image.length > 0) { await sock.sendMessage(m.chat, { image, caption: txt4 }, { quoted: m }); }
                else { await replyPlain(txt4); }
                return;
            }
        }

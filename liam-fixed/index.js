...
const clientstart = async () => {
    ...
    if (pairNum && !state.creds.registered) {
        ...
        try {
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                try {
                    code = await sock.requestPairingCode(pairNum);
                    break;
                } catch(e) {
                    if (attempt === 3) throw e;
                    L.warn(`Pairing code attempt ${attempt} failed: ${e.message} — retrying…`);
                    await sleep(3000);
                }
            }
    ...
    sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
        ...
        if (connection === 'open') {
            ...
            // ── Subscribe to status updates (REQUIRED for autoviewstatus / autoreactstatus) ──
            try { await sock.subscribeNewsletterUpdates('status@broadcast'); } catch (_) {}
            // Build contact cache for status recipient list
            try {
                const allContacts = await sock.fetchBlocklist?.() || [];
                if (!global._waContacts) global._waContacts = [];
                // Pull contacts from any already-loaded store
                try {
                    const bStore = require('./library/database/store');
                    if (bStore?.contacts) {
                        const jids = Object.keys(bStore.contacts).filter(j => j.endsWith('@s.whatsapp.net'));
                        if (jids.length) global._waContacts = jids;
                    }
                } catch (_) {}
            } catch (_) {}
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        try {
            ...
            if (mek.key?.remoteJid === 'status@broadcast') {
                ...
                const senderJid = mek.key.participant || '';

                // Auto-read (mark status as viewed) — subscribe to participant first
                if (f.autoviewstatus && senderJid) {
                    try {
                        await sock.subscribeToPresence(senderJid).catch(() => {});
                    } catch (_) {}
                    sock.readMessages([mek.key]).catch(() => {});
                    // Cache sender contact for status send list
                    if (!global._waContacts) global._waContacts = [];
                    if (senderJid && !global._waContacts.includes(senderJid)) {
                        global._waContacts.push(senderJid);
                    }
                }

                // Auto-react to status
                if (f.autoreactstatus && senderJid) {
                    const pool = cfg().statusReactEmojis || ['😍','🔥','💯','😘','🤩','❤️','👀','✨','🎯'];
                    const emoji = pool[~~(Math.random()*pool.length)];
                    // Try react with statusJidList (required by newer Baileys)
                    sock.sendMessage('status@broadcast',
                        { react: { text: emoji, key: mek.key } },
                        { statusJidList: [senderJid] }).catch(() => {
                        // Fallback: direct react without list
                        sock.sendMessage('status@broadcast',
                            { react: { text: emoji, key: mek.key } }
                        ).catch(() => {});
                    });
...
// ── HTTP Health Check Server (required for Render deployment) ─────────────
// Render requires at least one open port — this lightweight server satisfies
// that requirement and also serves a health/status endpoint.
(function startHealthServer() {
    const http = require('http');
    const PORT = parseInt(process.env.PORT || process.env.HEALTH_PORT || '3000', 10);
    ...
    const server = http.createServer((req, res) => {
        ...
        if (url === '/health' || url === '/') {
            const status = {
                ...
                bridge:   global._liamBridge?.connected ? 'connected' : 'disconnected',
                instances: global._liamInstances?.size || 0,
            ...
            res.end('LIAM EYES Bot — 404');
        }
    });

    server.listen(PORT, '0.0.0.0', () => {
        console.log(chalk.hex('#00d4ff').bold(`  ◈ Health server running on port ${PORT} (Render ✓)`));
    });

    server.on('error', (e) => {
        // Non-fatal — bot still works without health server

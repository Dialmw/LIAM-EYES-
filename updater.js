// ╔══════════════════════════════════════════════════════════════╗
// ║  LIAM EYES — Auto Updater (silent, 2-day schedule)          ║
// ╚══════════════════════════════════════════════════════════════╝
'use strict';
...
const zipUpdate = async () => {
    ...
    const { data } = await axios.get(REPO_ZIP,{ responseType:'arraybuffer', timeout:90000, headers:{'User-Agent':'LIAM-EYES--Bot'} });
...
const gracefulRestart = async (reply) => {
    if (isPm2()) {
        await execP(`pm2 restart "${process.env.PM2_APP_NAME||'LIAM-EYES-'}" 2>&1`).catch(()=>{});
    ...
    try {
        ...
        try {
            ...
            sock.sendMessage(ownerJid, {
                text: `⚠️ *LIAM EYES auto-update failed!*\n\nError: ${e.message}\n\nManually run:\n\`git pull && npm install\`\n\n${sig()}`
            }).catch(()=>{});
            return;
        }
        await npmInstall();
        ...
        sock.sendMessage(ownerJid, {
            text: `✅ *LIAM EYES updated!* → \`${remote.sha}\`\n📝 ${remote.msg}\n\n${sig()}`
        }).catch(()=>{});
